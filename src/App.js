...
//
  getPotentialBuyers() {
    ...
    }).catch(() => toast.error('Failed at finding Potential Buyers'));
...
//
  ...
  filterByMake() {
    ...
    axios.get(`https://joes-autos.herokuapp.com/api/vehicles?make=${make}`).then(results => {
      toast.success("Successfully got vehicle makes");
      this.setState({ 'vehiclesToDisplay': results.data });
    }).catch(() => toast.error('Failed at finding vehicle makes'));
    // setState with response -> vehiclesToDisplay
  }
  ...
  filterByColor() {
    ...
    axios.get(`https://joes-autos.herokuapp.com/api/vehicles?color=${color}`).then(results => {
      toast.success("Successfully got vehicle color");
      this.setState({ 'vehiclesToDisplay':results.data });
    }).catch(() => toast.error('Failed at finding vehicle color'));
    // setState with response -> vehiclesToDisplay
  }
//
  updatePrice(priceChange, id) {
    ...
    axios.put(`https://joes-autos.herokuapp.com/api/vehicles/${id}/${priceChange}`).then(results => {
      toast.success("Succesfully update price.");
      this.setState({ 'vehiclesToDisplay': results.data.vehicles });
...
//
  addCar() {
    ...
    axios.post('https://joes-autos.herokuapp.com/api/vehicles', newCar).then(results => {
      ...
      this.setState({ vehiclesToDisplay: results.data.vehicles });
    }).catch(() => toast.error('Failed at adding a vehicle. '));
    // setState with response -> vehiclesToDisplay
  }
  ...
  addBuyer() {
    ...
    axios.post('https://joes-autos.herokuapp.com/api/buyers', newBuyer).then(results => {
      toast.success('Successfully found buyer.');
      this.setState({ buyersToDisplay: results.data.buyers });
    }).catch(() => toast.error('Failed at adding buyer. '));
    // setState with response -> buyersToDisplay
  }
  ...
  nameSearch() {
    ...
    // axios (GET)
    axios.get(`https://joes-autos.herokuapp.com/api/vehicles?searchLetters${searchLetters}`).then(results => {
      toast.success("Successfully name search.");
      this.setState({ 'buyersToDisplay': results.data });
    }).catch(() => toast.error("Failed find name search"));
    // setState with response -> buyersToDisplay
  }
  ...
  byYear() {
    ...
    axios.get(`https://joes-autos.herokuapp.com/api/vehicles?year${year}`).then(results => {
      toast.success("Successfully got  by year.");
      this.setState({ 'vehiclesToDisplay': results.data });
